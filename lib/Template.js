...
module.exports = {
    ...
    list : function(topics) {
        ...
        var listText = '';
        for(key in list) {
            listText += `<li><a href="/?id=${list[key].id}">${sanitizeHTML(list[key].title)}</a></li>`;
        }
        return listText;
    },
    tag : function(authors, author_id) {
        ...
        for(author in authors) {
            ...
            tag += `<option value=${authors[author].id}${selected}>${sanitizeHTML(authors[author].name)}</option>`
        }
        tag += '</select>'
        return tag;
    },
    table : function(authors) {
        ...
        for(author in authors) {
            ...
            <td><a href="/author/update?id=${authors[author].id}">update</a></td>
            <td>
